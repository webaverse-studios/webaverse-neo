...
import { defaultCharacterName } from './endpoints.js'

const getCharacterFullPath = filename => './assets/json/characters/' + filename
...
export class CharacterSelectManager {
  ...
  getCharactersAsync () {
    if (!this.charactersLoadPromise) {
      this.charactersLoadPromise = loadJson(
        getCharacterFullPath('characters.json')
      )
    }
        ...
        await Promise.all(
          packs.map(async pack => {
            const characters = await Promise.all(
              pack.characters.map(characterName =>
