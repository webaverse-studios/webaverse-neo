...
import { decorateAnimation } from './util.mjs'
...
const initAnimationSystem = () => {
  ...
  for (const fileName in animations.index) {
    ...
    const animationPtr = avatarsWasmManager.physxWorker.createAnimation(
      animation.name,
      animation.duration
    )
    ...
    for (const spec of animationMappingConfig) {
      // correct interpolant index order
      const { animationTrackName: k } = spec
  ...
  const animationGroupDeclarations =
    avatarsWasmManager.physxWorker.initAnimationSystem()

  // get data back from wasm to js ------------------------------------------------

  // UseAnimationIndexes
  const useAnimationGroupDeclaration = animationGroupDeclarations.filter(
    n => n.name === 'use'
  )[0]
  useAnimationGroupDeclaration.animations.forEach(animationDeclaration => {
    UseAnimationIndexes[animationDeclaration.keyName] =
      animationDeclaration.index
  })

  // EmoteAnimationIndexes
  const emoteAnimationGroupDeclaration = animationGroupDeclarations.filter(
    n => n.name === 'emote'
  )[0]
  ...
  emoteAnimationGroupDeclaration.animations.forEach(animationDeclaration => {
    emoteAnimations[animationDeclaration.keyName] =
      animations.index[animationDeclaration.fileName]
  })
...
export const _createAnimation = avatar => {
  ...
  avatar.animationAvatarPtr =
    avatarsWasmManager.physxWorker.createAnimationAvatar(avatar.mixerPtr)
}
...
export const _updateAnimation = (avatar, now, timeDiff) => {
  ...
  const updateValues = () => {
    const forwardFactor =
      1 - MathUtils.clamp(Math.abs(angle) / (Math.PI / 2), 0, 1)
    const backwardFactor =
      1 - MathUtils.clamp((Math.PI - Math.abs(angle)) / (Math.PI / 2), 0, 1)
    const leftFactor =
      1 - MathUtils.clamp(Math.abs(angle - Math.PI / 2) / (Math.PI / 2), 0, 1)
    const rightFactor =
      1 - MathUtils.clamp(Math.abs(angle - -Math.PI / 2) / (Math.PI / 2), 0, 1)
    ...
    avatarsWasmManager.physxWorker.updateAnimationAvatar(
      avatar.animationAvatarPtr,
      values,
      timeDiff
    )
  ...
  let resultValues
  const doUpdate = () => {
    resultValues = avatarsWasmManager.physxWorker.updateAnimationMixer(
      avatar.mixerPtr,
      now,
      nowS
    )
