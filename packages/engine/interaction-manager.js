...
const _updateGrabbedObject = (
  ...
  if (
    collNonGrabbedObj &&
    !!downCollision &&
    collisionIsWithinOffset &&
    collisionIsAboveGround
  ) {
    localVector5.copy(localVector6).sub(physicalOffset)
  }

  const objectOverlapsVertically =
...
class PhysicsTarget extends THREE.Object3D {
  ...
  setTarget (targetApp, targetPhysicsObject) {
    ...
    if (targetApp && targetPhysicsObject) {
      ...
      physicsMesh.matrixWorld.decompose(
      ...
      if (!this.editMode) {
...

        {
          ...
          if (collision) {
            ...
            const physicsObject =
              this.physicsTracker.getPhysicsObjectByPhysicsId(physicsId)
