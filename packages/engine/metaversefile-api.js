...
import domRenderEngine from './dom-renderer'
    ...
    if (app) {
      ...
      physicsScene.addCookedHeightfieldGeometry =
        (addCookedHeightfieldGeometry =>
          function () {
            const physicsObject = addCookedHeightfieldGeometry.apply(
              this,
              arguments
            )
            // app.add(physicsObject);
  ...
  useInternals () {
    if (!iframeContainer && !isWorker) {
      ...
      iframeContainer.getFov = () =>
        camera.projectionMatrix.elements[5] * (globalThis.innerHeight / 2)
