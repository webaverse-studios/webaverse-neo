...
export function makeAvatar (app, environmentManager) {
  if (app?.appType === 'vrm') {
    if (!app.avatarQuality) {
      ...
      // TODO: FIX THIS. THIS IS JUST SO IT STOPS THROWING ERRORS
      let avatarManager = new AvatarManager()
      const avatarQuality = avatarManager.makeQuality(app.modules[0])
...
export function applyFacePoseToAvatar (character, rig) {
  const facePoseActions = character.actionManager
    .getActionsArray()
