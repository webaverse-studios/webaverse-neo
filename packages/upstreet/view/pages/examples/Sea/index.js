 ...
 * @param root0.encryptedText
 * @param root0.decryptedText
 */
async function configureStreams({ inputText, encryptedText, decryptedText }) {
  ...
  inputText.map( async ( text ) =>
    encryptedText( await sea.encrypt( text, await sea.secret( bob.epub, alice )))
  )

  // Decrypt text.
  encryptedText.map( async ( text ) =>
    decryptedText( await sea.decrypt( text, await sea.secret( alice.epub, bob )))
  )
