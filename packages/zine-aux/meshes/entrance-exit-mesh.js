...
export class EntranceExitMesh extends THREE.Mesh {
  constructor({ entranceExitLocations }) {
    ...
    const colors = new Float32Array(
      baseGeometry.attributes.position.array.length
    )
    ...
    entranceExitMesh.setHighlights = ( highlights ) => {
      for ( let i = 0; i < highlights.length; i++ ) {
        ...
        for (
          let j = 0;
          j < baseGeometry.attributes.color.array.length / 3;
          j++
        ) {
          const baseJ =
            i * baseGeometry.attributes.color.array.length + j * 3
          // set rgb
