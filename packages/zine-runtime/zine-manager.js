...
} from '@webaverse-studios/zine/zine-camera-utils.js'
import {
  floorNetResolution,
  heightfieldScale,
  panelSize,
...
import {
  ZineStoryboard,
  zineMagicBytes,
} from '@webaverse-studios/zine/zine-format.js'
import {
  getDepthFloatsFromPointCloud,
  getDoubleSidedGeometry,
  getGeometryHeights,
  reconstructPointCloudFromDepthField,
} from '@webaverse-studios/zine/zine-geometry-utils.js'
import { getFloorNetPhysicsMesh } from '@webaverse-studios/zine/zine-mesh-utils.js'
import { ZineRenderer } from '@webaverse-studios/zine/zine-renderer.js'
import { EntranceExitMesh } from '@webaverse-studios/zine-aux/meshes/entrance-exit-mesh.js'
import { StoryTargetMesh } from '@webaverse-studios/zine-aux/meshes/story-target-mesh.js'

import { PanelRuntimeItems } from './actors/zine-item-actors.js'
import { PanelRuntimeMobs } from './actors/zine-mob-actors.js'
import { PanelRuntimeNpcs } from './actors/zine-npc-actors.js'
import { PanelRuntimeOres } from './actors/zine-ore-actors.js'
import {
...
class PanelRuntimeInstance extends THREE.Object3D {
  constructor( panel, { zineCameraManager, physics, localPlayer, ctx }) {
    super()
  ...
  #init() {
    ...
    const { entranceExitLocations } = zineRenderer.metadata
    zineRenderer.addEventListener(
      'load',
      ( e ) => {
        this.dispatchEvent({
          type: 'load',
    ...
    // scene physics
    {
      ...
      const scenePhysicsMesh2 = new THREE.Mesh(
        geometry2,
        scenePhysicsMesh.material
      )
    ...
    // floor net physics
    {
      const [floorWidth, floorHeight] = floorResolution
    ...
    this.wallPhysicsObjects = []
    {
      for ( let i = 0; i < wallPlaneMeshes.length; i++ ) {
        ...
        const { position: centerPoint, quaternion: planeQuaternion } =
          _getTransform()
        const dynamic = false
  ...
  alignEntranceToFloor(
    floorPosition,
    exitWorldLocation,
    entranceLocalLocation
  ) {
    // transform the zine renderer
    this.zineRenderer.alignEntranceToFloorPosition(
      floorPosition,
      exitWorldLocation,
      entranceLocalLocation
    )
    ...
    if ( this.selected ) {
      ...
      const _updateEntranceExitHighlights = () => {
        ...
        if ( localPlayer.loaded ) {
          const { capsuleWidth: capsuleRadius, capsuleHeight } =
          ...
          if ( intersectionIndex !== -1 ) {
            const entranceExit =
              zineRenderer.metadata.entranceExitLocations[intersectionIndex]
            const { panelIndex, entranceIndex } = entranceExit
...
export class PanelInstanceManager extends THREE.Object3D {
  constructor(
    storyboard,
    { zineCameraManager, physics, localPlayer, spawnManager, ctx }
  ) {
    super()
  ...
  #init() {
    const { zineCameraManager, physics, localPlayer, ctx } = this
    const panelOpts = {
      zineCameraManager,
      physics,
      // physicsTracker,
    ...
    this.loadPromise = ( async () => {
      ...
      await Promise.all(
        panelInstances.map(( panelInstance ) => panelInstance.waitForLoad())
      )
    ...
    for ( let i = 0; i < this.panelInstances.length - 1; i++ ) {
      ...
      this.physics.setTransform(
        nextPanelInstance.floorNetPhysicsObject,
        false
      )
    }
      ...
      panelInstance.addEventListener( 'transition', ( e ) => {
        ...
        if ( nextPanelIndex >= 0 && nextPanelIndex < panels.length ) {
          ...
          const { entranceExitLocations } =
            currentPanelInstance.zineRenderer.metadata
          ...
          const { entranceExitLocations: nextEntranceExitLocations } =
            nextPanelInstance.zineRenderer.metadata
          const nextEntranceLocation =
            nextEntranceExitLocations[nextEntranceIndex]
          ...
          const scale = new THREE.Vector3().fromArray( scaleArray )
          const mainMatrixWorld = new THREE.Matrix4().compose(
            position,
            quaternion,
            scale
          )
          ...
          if ( signedDistance < 0 ) {
            ...
            /*if (window.lol) */ {
              ...
              this.zineCameraManager.transitionLockCamera(
                nextPanelInstance.zineRenderer.camera,
  ...
  spawn() {
    ...
    const position = new THREE.Vector3().fromArray(
      cameraEntranceLocation.position
    )
    const quaternion = new THREE.Quaternion().fromArray(
      cameraEntranceLocation.quaternion
    )
