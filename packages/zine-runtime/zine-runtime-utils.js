...
export const getCapsuleIntersectionIndex = (
  ...
  for ( let i = 0; i < entranceExitLocations.length; i++ ) {
    ...
    const boxPosition = new THREE.Vector3()
      .fromArray( eel.position )
      .add(
        new THREE.Vector3(
          0,
          entranceExitHeight / 2,
          entranceExitDepth / 2
    ...
    localMatrix
      .compose( boxPosition, boxQuaternion, boxSize )
      .premultiply( matrixWorld )
      .decompose( boxPosition, boxQuaternion, boxSize )

    const capsulePosition = position
      .clone()
...
export function capsuleIntersectsBox(
  ...
  const capsuleLine = new THREE.Line3(
    capsulePosition
      .clone()
      .add( new THREE.Vector3( 0, capsuleHeight / 2 + capsuleRadius, 0 )),
    capsulePosition
      .clone()
      .add( new THREE.Vector3( 0, -capsuleHeight / 2 - capsuleRadius, 0 ))
  )
  capsuleLine.start
    .sub( boxPosition )
    .applyQuaternion( boxQuaternion.clone().invert())
  capsuleLine.end
    .sub( boxPosition )
    .applyQuaternion( boxQuaternion.clone().invert())
  // then, get the intersection in the box's local space
  return capsuleIntersectsAABB( capsuleLine, capsuleRadius, boxSize )
}
/**
 *
 * @param capsuleLine
