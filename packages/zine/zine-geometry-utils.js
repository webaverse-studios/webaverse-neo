...
import { colors, detectronColors, rainbowColors } from './zine-colors.js'
import { pointcloudStride } from './zine-constants.js'
 ...
 * @param pz
 */
export function bilinearInterpolate( values, width, height, px, pz ) {
...
export function bilinearInterpolateChecked(
  ...
  if ( validPoints.length > 0 ) {
    ...
    // interpolate
    return a * fx1 * fz1 + b * fx * fz1 + c * fx1 * fz + d * fx * fz
  } else {
    return null
  }
}
export const bilinearInterpolate3 = (() => {
  ...
  return ( values, width, height, px, pz, targetVector ) => {
    // first, compute the sample coordinates:
    const x = Math.floor( px * ( width - 1 ))
    const z = Math.floor( pz * ( height - 1 ))
    const x1 = Math.min( x + 1, width - 1 )
...
/**
 *
 * @param srcArrayBuffer
 * @param width
 * @param height
 * @param scaleFactor
 * @param dstFloat32Array
 */
...
/**
 *
 * @param pointCloudArrayBuffer
 * @param width
 * @param height
 * @param matrixWorld
 */
export function getBoundingBoxFromPointCloud(
...
export function depthFloat32ArrayToPositionAttributeArray(
  ...
  for ( let i = 0; i < depthFloat32Array.length; i++ ) {
    ...
    const worldPoint = setCameraViewPositionFromViewZ(
      x,
      y,
      viewZ,
      camera,
...
export function depthFloat32ArrayToOrthographicPositionAttributeArray(
  ...
  for ( let i = 0; i < depthFloat32Array.length; i++ ) {
    ...
    const viewZ = depthFloat32Array[i]
    const worldPoint = setCameraViewPositionFromOrthographicViewZ(
      x,
      y,
      viewZ,
      camera,
...
export const getGepthFloatsFromGeometryPositions = ( geometryPositions ) => {
  const newDepthFloatImageData = new Float32Array(
    geometryPositions.length / 3
  )
...
export const getDepthFloatsFromPointCloud = (
  pointCloudArrayBuffer,
  width,
  height
) => {
...
export const getDepthFloatsFromIndexedGeometry = ( geometry ) =>
  getGepthFloatsFromGeometryPositions( geometry.attributes.position.array )

//

export const snapPointCloudToCamera = (
  pointCloudArrayBuffer,
  width,
  height,
  camera
  ...
  const frustum = localFrustum.setFromProjectionMatrix(
...
export function decorateGeometryTriangleIds( geometry ) {
  const triangleIdAttribute = new THREE.BufferAttribute(
...
export const setCameraViewPositionFromViewZ = (
  ...
  const clipW =
    projectionMatrix.elements[2 * 4 + 3] * viewZ +
    projectionMatrix.elements[3 * 4 + 3]
...
export const setCameraViewPositionFromOrthographicViewZ = (
  x,
  y,
  viewZ,
  camera,
...
export const getGeometryHeights = (
  geometry,
  width,
  height,
  heightfieldScale
