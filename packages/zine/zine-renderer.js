...
import { mainImageKey } from './zine-data-specs.js'
import {
...
import { makeDefaultCamera, range } from './zine-utils.js'

const oneVector = new THREE.Vector3( 1, 1, 1 )
const upVector = new THREE.Vector3( 0, 1, 0 )
const y180Quaternion = new THREE.Quaternion().setFromAxisAngle(
      ...
      fragmentShader: `\
        ...
        void main() {
          ...
          if (uEraser == 1) {
            ...
            if (isSelected) {
              if (uMouseDown == 1) {
                gl_FragColor.rgb = vec3(${new THREE.Color( 0xff3333 )
        .toArray()
        .join( ', ' )});
              } else {
...
class SceneMesh extends THREE.Mesh {
  constructor({
    ...
    const segmentArray = reconstructValueMaskFromLabelsIndices(
      segmentLabels,
      segmentLabelIndices
    )
    geometry.setAttribute(
      'segment',
      new THREE.BufferAttribute( segmentArray, 1 )
    )
    const segmentColor = getColorArrayFromValueArray( segmentArray )
    geometry.setAttribute(
      'segmentColor',
      new THREE.BufferAttribute( segmentColor, 3 )
    )
    const planeArray = reconstructValueMaskFromLabelsIndices(
      planeLabels,
      planeLabelIndices
    )
...
class CapSceneMesh extends THREE.Mesh {
  constructor({ edgeDepths, map, width, height }) {
    const numTriangles = ( width - 1 ) * 2 + ( height - 1 ) * 2
...

    {
      ...
      for ( let i = 0; i < numTriangles; i++ ) {
        positions[centerPointVertexStartIndex * 3 + i * 3 + 0] =
          centerPoint.x
        positions[centerPointVertexStartIndex * 3 + i * 3 + 1] =
          centerPoint.y
        positions[centerPointVertexStartIndex * 3 + i * 3 + 2] =
          centerPoint.z
      ...
      for ( const edgeSpec of [
        ...
        const { pointsArray, uvStart, uvEnd, flip } = edgeSpec
        // indices
...
class FloorNetMesh extends THREE.Mesh {
  constructor() {
    ...
    floorNetMesh.setGeometry = ({ floorNetDepths, floorNetCamera }) => {
...
class EdgeDepthMesh extends THREE.InstancedMesh {
  constructor({ edgeDepths, width, height }) {
    const depthCubesGeometry = new THREE.BoxGeometry( 0.01, 0.01, 0.01 )
    // instanced color attribute
    const maxSize = width + width + height + height + 2 * 4 // min/max for each edge
    const colors = new Float32Array( maxSize * 3 )
    depthCubesGeometry.setAttribute(
      'color',
    ...
    ;[
      ...
      for ( let i = 0; i < ps.length; i++ ) {
        ...
        localColor.setHex( 0x0000ff ).toArray( colors, index * 3 )
        index++
    ...
    ;[
      ...
      depthCubesMesh.setMatrixAt( index, localMatrix )
      localColor.setHex( 0xff0000 ).toArray( colors, index * 3 )
      index++
      depthCubesMesh.count++
...
class WallPlaneMesh extends THREE.Mesh {
  constructor( wallPlane ) {
    ...
    const geometry = new THREE.PlaneGeometry( 2, 2 ).rotateY( Math.PI / 2 ) // match physx convention
    const material = new THREE.MeshPhongMaterial({
      color,
      transparent: true,
      opacity: 0.3,
...
class LightMesh extends THREE.Mesh {
  constructor({ sphericalHarmonics: sh }) {
    const _addMonocolor = ( geometry, v ) => {
      const monocolor = new Float32Array(
        geometry.attributes.position.array.length / 3
      ).fill( v )
      geometry.setAttribute(
        'monocolor',
        new THREE.BufferAttribute( monocolor, 1 )
      )
    }
    ...
    const lightDirR = new THREE.Vector3(
      -sh1_r.x,
      -sh0_r.y,
      sh0_r.z
    ).normalize()
    const lightDirG = new THREE.Vector3(
      -sh1_g.x,
      -sh0_g.y,
      sh0_g.z
    ).normalize()
    const lightDirB = new THREE.Vector3(
      -sh1_b.x,
      -sh0_b.y,
      sh0_b.z
    ).normalize()
    ...
    const sh0_light = new THREE.Vector3(
      0.282094791,
      -0.488602511 * lightDir.y,
      0.488602511 * lightDir.z
    )
    const sh1_light = new THREE.Vector3(
      -0.488602511 * lightDir.x,
      1.092548431 * lightDir.y * lightDir.x,
      -1.092548431 * lightDir.y * lightDir.z
    )
    const sh2_light = new THREE.Vector3(
      0.315391565 * ( 3.0 * lightDir.z * lightDir.z - 1.0 ),
    ...
    const denom =
      sh0_light.dot( sh0_light ) +
      sh1_light.dot( sh1_light ) +
      sh2_light.dot( sh2_light )
    ...
    const _makeGeometry = ({ planeGeometry, rodGeometry, scale }) => {
      const frontGeometries = [
        planeGeometry.clone().scale( scale, scale, scale ),
      ].concat(
        [
          [-1, -1],
          [-1, 1],
      ...
      const geometry =
        BufferGeometryUtils.mergeBufferGeometries( frontGeometries )
      return geometry
    }
...
export class ZineRenderer extends EventTarget {
  constructor({ panel, alignFloor = false }) {
    super()
  ...
  #listen() {
    ...
    layer1.addEventListener( 'update', ( e ) => {
      ...
      const transformKeys = ['position', 'quaternion', 'scale']
  ...
  #syncTransformToData() {
    // update scene transform to match panel data
    const layer1 = this.panel.getLayer( 1 )
    const position = layer1.getData( 'position' )
    const quaternion = layer1.getData( 'quaternion' )
    const scale = layer1.getData( 'scale' )
   ...
   * @returns {Promise} Resolves to an image
   */
  ...
  connect( targetZineRenderer, exitIndex = 1, entranceIndex = 0 ) {
    ...
    const exitMatrixWorld = exitMatrix
      .clone()
      .premultiply( this.transformScene.matrixWorld )
    exitMatrixWorld.decompose( localVector, localQuaternion, localVector2 )
    exitMatrixWorld.compose( localVector, localQuaternion, oneVector )

    const entranceLocation =
      targetZineRenderer.metadata.entranceExitLocations[entranceIndex]
    ...
    const entranceMatrixWorld = entranceMatrix
      .clone()
      .premultiply( targetZineRenderer.transformScene.matrixWorld )
    entranceMatrixWorld.decompose( localVector, localQuaternion, localVector2 )
    entranceMatrixWorld.compose( localVector, localQuaternion, oneVector )
  ...
  alignEntranceToFloorPosition(
    ...
    const exitDropMatrixWorld = new THREE.Matrix4().compose(
      floorPosition,
      localQuaternion
        .fromArray( exitWorldLocation.quaternion )
        .premultiply( y180Quaternion ),
      oneVector
    )

    const entranceLocalMatrixInverse = new THREE.Matrix4()
      .compose(
    ...
    const transformMatrix = entranceLocalMatrixInverse
      .clone()
