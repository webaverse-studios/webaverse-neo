import { zbencode, zbdecode, zbclone } from './encoding.js'
import { align4 } from './util.js'
...
const _makeDataView = uint8Array =>
  new DataView(uint8Array.buffer, uint8Array.byteOffset, uint8Array.byteLength)
const _parseBoundEvent = encodedEventData => {
  ...
  if (Cons) {
    ...
    console.warn(
      'could not parse bound event due to incorrect method',
      method,
      ZEVENT_CONSTRUCTORS
    )
    return null
...
const _getHistoryMethod = (historyData, historyOffsets, historyIndex) => {
  const dataView = _getHistoryDataView(
    historyData,
    historyOffsets,
    historyIndex
  )
...
const _getHistoryResolvePriority = (
  historyData,
  historyOffsets,
  historyIndex
) => {
...
const _getHistoryKeyPathBuffer = (
  historyData,
  historyOffsets,
  historyIndex
) => {
...
const _parentWasSet = (
  ...
  for (let i = historyStartIndex; i < historyEndIndex; i++) {
    ...
    if (historyMethod !== ZNullEvent.METHOD) {
      const historyKeyPathBuffer = _getHistoryKeyPathBuffer(
        historyData,
        historyOffsets,
        i
      )
  ...
  for (let i = historyStartIndex; i < historyEndIndex; i++) {
    ...
    if (
      ...
      if (_keyPathEquals(historyKeyPathBuffer, event.getKeyPathBuffer())) {
        // if it is the same keypath
        conflictFound = true

        const historyResolvePriority = _getHistoryResolvePriority(
          historyData,
          historyOffsets,
          i
        )
...
class TransactionCache {
  constructor (
    ...
    resolvePriority = doc.resolvePriority,
    events = [],
    observerEvents = []
  ) {
    this.doc = doc
  ...
  static deserializeUpdate (uint8Array) {
    ...
    for (let i = 0; i < numEvents; i++) {
      ...
      const encodedEventData = new Uint8Array(
        uint8Array.buffer,
        uint8Array.byteOffset + index,
        eventLength
      )
...
class ZDoc extends ZEventEmitter {
  ...
  pushHistory (resolvePriority, event) {
    ...
    const eventByteLength = event.serializeHistory(
      resolvePriority,
      eventTargetBuffer
    )
  ...
  setClockState (clock, state) {
    ...
    const _emitAddEvents = state => {
      const _recurse = binding => {
        ...
        if (impl?.isZDoc) {
          ...
          if (impl.length > 0) {
            const rawValues = impl.binding.e.map(
              value => bindingsMap.get(value) ?? value
            )
    ...
    const _remapState = (oldState, newState) => {
      ...
      const _recurseRemapState = (newBinding, keyPath) => {
        ...
        const newParent =
          keyPath.length > 0 ? _lookupKeyPathParent(newState, keyPath) : null
        let oldImpl
  ...
  getImplByKeyPathParent (keyPath, keyTypes) {
    ...
    for (let i = 0; i < keyPath.length - 1; i++) {
      ...
      const child = (() => {
        switch (keyType) {
          case KEY_TYPES.ARRAY:
            return impl.get(key, ZArray)
          case KEY_TYPES.MAP:
            return impl.get(key, ZMap)
          case KEY_TYPES.VALUE:
            return impl.get(key)
          case KEY_TYPES.ELEMENT | KEY_TYPES.ARRAY:
            return impl.getId(key, ZArray)
          case KEY_TYPES.ELEMENT | KEY_TYPES.MAP:
            return impl.getId(key, ZMap)
          case KEY_TYPES.ELEMENT | KEY_TYPES.VALUE:
            return impl.getId(key)
          default:
            return 'lol'
        }
      })()
      ...
      if (parentBinding) {
        ...
        if (parentImpl) {
          if (parentImpl.isZDoc) {
            ...
            const impl = bindingsMap.get(binding)
            const type =
              (_getImplKeyType(impl) || KEY_TYPES.VALUE) | KEY_TYPES.ELEMENT
            keyPath.push(zid)
            keyTypes.push(type)
...
class ZMap extends ZObservable {
  ...
  set (k, v) {
    ...
    keyPath.push(k)
    keyTypes.push(keyType)
    const event = new ZMapSetEvent(keyPath, keyTypes, v)
  ...
  delete (k) {
    ...
    const event = new ZMapDeleteEvent(keyPath, keyTypes)
    event.bindToImpl(this)
    if (this.doc) {
      this.doc.pushTransaction()
      this.doc.transactionCache.pushEvent(event)
...
class ZArray extends ZObservable {
  ...
  push (arr) {
    ...
    keyPath.push(zid)
    keyTypes.push(keyType)
    const event = new ZArrayPushEvent(keyPath, keyTypes, arr)
    event.bindToImpl(this)
    if (this.doc) {
      this.doc.pushTransaction()
      this.doc.transactionCache.pushEvent(event)
  ...
  delete (index, length = 1) {
    ...
    const event = new ZArrayDeleteEvent(keyPath, keyTypes)
    event.bindToImpl(this)
    if (this.doc) {
      this.doc.pushTransaction()
      this.doc.transactionCache.pushEvent(event)
...
class ZEvent {
  ...
  getObserverEvent () {
    ...
    if (actionSpec) {
      ...
      const added = new Set(
        /add|update/.test(actionSpec.action)
          ? [
              {
                content: {
                  type: value
    ...
    if (this.keyPathBuffer === null) {
      ...
      for (let i = 0; i < this.keyPath.length; i++) {
        ...
        const { written } = textEncoder.encodeInto(
          key,
          uint8ArrayBuffer.subarray(index)
        )
        index += written
...
class ZMapSetEvent extends ZMapEvent {
  ...
  static deserializeUpdate (uint8Array) {
    ...
    const kpjbLength = dataView.getUint32(index, true)
    index += Uint32Array.BYTES_PER_ELEMENT
    const keyPath = _parseKeyPathBuffer(
      new Uint8Array(
        uint8Array.buffer,
        uint8Array.byteOffset + index,
        kpjbLength
    ...
    return new this(keyPath, keyTypes, value)
  }
}
class ZMapDeleteEvent extends ZMapEvent {
  ...
  static deserializeUpdate (uint8Array) {
    ...
    return new this(keyPath, keyTypes, key)
  }
}
class ZArrayPushEvent extends ZArrayEvent {
  ...
  static deserializeUpdate (uint8Array) {
    ...
    return new this(keyPath, keyTypes, arr)
  }
}
class ZArrayDeleteEvent extends ZArrayEvent {
  ...
  static deserializeUpdate (uint8Array) {
    ...
    return new this(keyPath, keyTypes)
  }
}
