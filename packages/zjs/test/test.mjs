...
describe( 'stress test', function () {
  ...
  class Simulation {
    ...
    clone() {
      ...
      for ( let i = 0; i < clients.length; i++ ) {
        const serverToOldClientPipe = server.pipes.find(
          ( pipe ) => pipe.destination === this.clients[i]
        )
        serverToOldClientPipe.destination = clients[i]

        const clientToOldServerPipe = clients[i].pipes.find(
          ( pipe ) => pipe.destination === this.server
        )
        clientToOldServerPipe.destination = server
      }

      return new Simulation( server, clients )
    }
    flush() {
      const _flushClients = () => {
        for ( const client of this.clients ) {
    ...
    verify() {
      const serverWorldAppManagerAppArray =
        this.server.doc.getArray( 'world.apps' )
      ...
      const serverPlayersMap = new Map(
        serverPlayersArray2.map(( player ) => {
          return [player.get( 'playerId' ), player.toJSON()]
        })
      )

      for ( const client of this.clients ) {
        const clientWorldAppManagerAppArray =
          client.doc.getArray( 'world.apps' )
        ...
        const clientPlayersMap = new Map(
          clientPlayersArray2.map(( player ) => {
            return [player.get( 'playerId' ), player.toJSON()]
          })
        )

        assert.deepEqual(
          clientWorldAppManagerAppArray.toJSON(),
          serverWorldAppManagerAppArray.toJSON()
        )
  ...
  class WorldView extends EventTarget {
    constructor( doc = new Z.Doc()) {
      ...
      playersArray.observe(( e ) => {
        ...
        for ( const d of e.changes.deleted.values()) {
          const {
            content: { type: oldPlayerMap },
          } = d
          const playerId = oldPlayerMap.get( 'playerId' )
          const oldPlayerIndex = this.remotePlayers.findIndex(
    ...
    unpipe( packetDestination ) {
      const index = this.pipes.findIndex(
        ( pipe ) => pipe.destination === packetDestination
      )
      if ( index !== -1 ) {
  ...
  class ServerWorldView extends WorldView {
    ...
    clearPlayer( playerId ) {
      const playerIndex = this.remotePlayers.findIndex(
        ( player ) => player.playerId === playerId
      )
      if ( playerIndex !== -1 ) {
        const playersArray = this.getPlayersArray()
  ...
  class ClientWorldView extends WorldView {
    ...
    update() {
      ...
      if ( this.isBound ) {
        if ( r < 1 / 3 ) {
          const newPosition = Float32Array.from([
            rngndc(),
            rngndc(),
            rngndc(),
          ])
